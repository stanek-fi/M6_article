...
par <- setNames(lapply(par[["sinusoidal"]], function(x) x[[1]]), sapply(par[["sinusoidal"]], function(x) names(x)))

sourcePath <- file.path("src", "MtMs")
sapply(file.path(sourcePath, list.files(sourcePath)), source)
sourcePath <- file.path("src", "sinusoidal", "helpers")
sapply(file.path(sourcePath, list.files(sourcePath)), source)
temp_file_path <- file.path("models", "sinusoidal", "temp")
dir.create(temp_file_path, showWarnings = FALSE)

for(K_train in par$K_train) {
    set.seed(par$seed)
    ...
    train_in <- rbind_list(lapply(seq_along(thetas_in), function(i) DGP(K_train, thetas_in[[i]], i)))
    test_in <- rbind_list(lapply(seq_along(thetas_in), function(i) DGP(K_test, thetas_in[[i]], i)))
    validation_in <- rbind_list(lapply(seq_along(thetas_in), function(i) DGP(par$K_validation, thetas_in[[i]], i)))
    train_out <- rbind_list(lapply(seq_along(thetas_out), function(i) DGP(K_train, thetas_out[[i]], i)))
    test_out <- rbind_list(lapply(seq_along(thetas_out), function(i) DGP(K_test, thetas_out[[i]], i)))
    ...
    for (m in seq_len(par$M_out)) {
        ...
        losses_mesa[m] <- as.array(criterion(mesa_model(tensorValidation_m$x), tensorValidation_m$y))
    ...
    print(metrics)
    path <- file.path("outputs", "sinusoidal")
    dir.create(path, showWarnings = FALSE)
    write_yaml(metrics, file.path(path, str_c("metrics_K_", K_train, ".yaml")))
    ...
    d <- do.call(rbind, lapply(seq_len(ncol(mesa_mat_quant)), function(mesa_i) {
        do.call(rbind, lapply(seq_len(nrow(mesa_mat_quant)), function(q) {
            ...
            data.table(
                ...
                mesa_other = round(mesa_state[-mesa_i], 3),
    ...
    p1 <- ggplot(d[mesa_par == "mesa_1"], aes(x = x, y = y, colour = mesa)) +
        geom_line() +
        labs(colour = TeX(r"($\theta[1]$)"), y = TeX(paste0(r"($\hat{y}=f_{\omega}(x;[)", r"(\theta[1])", ",", d[mesa_par == "mesa_1", unique(mesa_other)], r"(]^{T})$)")))
    p2 <- ggplot(d[mesa_par == "mesa_2"], aes(x = x, y = y, colour = mesa)) +
        geom_line() +
        labs(colour = TeX(r"($\theta[2]$)"), y = TeX(paste0(r"($\hat{y}=f_{\omega}(x;[)", d[mesa_par == "mesa_2", unique(mesa_other)], ",", r"(\theta[2])", r"(]^{T})$)")))
    p <- plot_grid(p1, p2, ncol = 1)
    path <- file.path("outputs", "sinusoidal")
    dir.create(path, showWarnings = FALSE)
    ggsave(file.path(path, str_c("prediction_functions_K_", K_train, ".pdf")), plot = p, height = 7, width = 7)
